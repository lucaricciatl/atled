...
private:
    ...
    Coordinates2D ApproximateForce(const Coordinates2D& entityPos, const Coordinates2D& centerOfMass,
        float massNode, float massEntity) const;

    // Insert an entity into the quadtree
    void InsertEntity(QuadTreeNode* node, size_t entityIndex, const Coordinates2D& pos,
        float xMin, float xMax, float yMin, float yMax, size_t maxEntities = 4);
