...
NBodySimulation::NBodySimulation(std::vector<std::shared_ptr<Entity>> entities) : entities(std::move(entities)) {
    InitializeBodyData();
}
...
void NBodySimulation::UpdateBodies(const std::vector<Coordinates2D>& forces, float dt) {
    for (size_t i = 0; i < entities.size(); ++i) {
        ...
        bodies[i].velocity = Coordinates2D((bodies[i].velocity.GetX() + acceleration.GetX() * dt) * dampingFactor,
...
void NBodySimulation::InsertEntity(QuadTreeNode* node, size_t entityIndex, const Coordinates2D& pos, float xMin,
                                   float xMax, float yMin, float yMax, size_t maxEntitiesPerNode) {
    ...
    InsertEntity(node->children[quadrant], entityIndex, pos, node->children[quadrant]->xMin,
                 node->children[quadrant]->xMax, node->children[quadrant]->yMin, node->children[quadrant]->yMax,
                 maxEntitiesPerNode);
}
...
void NBodySimulation::SubdivideNode(QuadTreeNode* node) {
    ...
    node->children[0] = new QuadTreeNode({(node->xMin + xMid) / 2, (yMid + node->yMax) / 2}, node->xMin, xMid, yMid,
                                         node->yMax);  // Top-left
    node->children[1] = new QuadTreeNode({(xMid + node->xMax) / 2, (yMid + node->yMax) / 2}, xMid, node->xMax, yMid,
                                         node->yMax);  // Top-right
    node->children[2] = new QuadTreeNode({(node->xMin + xMid) / 2, (node->yMin + yMid) / 2}, node->xMin, xMid,
                                         node->yMin, yMid);  // Bottom-left
    node->children[3] = new QuadTreeNode({(xMid + node->xMax) / 2, (node->yMin + yMid) / 2}, xMid, node->xMax,
                                         node->yMin, yMid);  // Bottom-right
...
void NBodySimulation::ComputeForcesFMM(const QuadTreeNode* node, size_t entityIndex, const Coordinates2D& entityPos,
    ...
    if (node->isLeaf &&
        (node->entities.size() == 0 || (node->entities.size() == 1 && node->entities[0] == entityIndex))) {
    ...
    if ((node->isLeaf && node->entities.size() <= 1) || (size / distance) < theta) {
        ...
        Coordinates2D approxForce =
            ApproximateForce(entityPos, node->centerOfMass, node->mass, bodies[entityIndex].mass);
...
void NBodySimulation::SetDampingFactor(float damping) { dampingFactor = damping; }

void NBodySimulation::SetMinDistanceThreshold(float threshold) { minDistanceThreshold = threshold; }
}  // namespace nbody