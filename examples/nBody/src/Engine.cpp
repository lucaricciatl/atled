...
void Engine::OnStart() {
  ...
  ctx->SetTitle("Window");
  auto configs =
      graphics::GraphicsConfig({FLAG_WINDOW_RESIZABLE, FLAG_VSYNC_HINT,
  ...
  for (auto &circle : simulation.GetCircles()) {
    graphicsManager->AddShape(1, std::dynamic_pointer_cast<graphics::Model2D>(
...
void Engine::OnShutdown() {
  ...
  // Perform any cleanup if needed (e.g., deallocating resources, saving state,
  // etc.)
}
...
void Engine::OnUpdate() {
  ...
  if (inputManager->IsKeyDown(KEY_W)) {
    cameraManager->SetCameraTargetY(cameraManager->GetCameraTargetY() -
                                    1); // Move view up
  }
  ...
  if (inputManager->IsKeyDown(KEY_A)) {
    cameraManager->SetCameraTargetX(cameraManager->GetCameraTargetX() -
                                    1); // Move view left
  }
  ...
  // Apply target-based adjustments to Entitys
