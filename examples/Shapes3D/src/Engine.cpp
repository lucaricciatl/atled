...
std::vector<std::shared_ptr<Model3D>>
    shapes;                                 // To store references to the cubes
...
void Engine::OnUpdate() {
  ...
  const float radius = 120.0f;        // Distance from the Entity
  ...
  cameraManager->SetCameraTarget(
      {25, 25, 25}); // Keep the target fixed on the Entity
  ...
  for (auto &cube : shapes) {
    if (cube) {
      ...
      int green =
          static_cast<int>((sin(phase + 2.0f) + 1) * 127.5); // Offset for green
      int blue =
          static_cast<int>((sin(phase + 4.0f) + 1) * 127.5); // Offset for blue
